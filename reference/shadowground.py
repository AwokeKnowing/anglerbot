...
def pointcloud(out, verts, texcoords, color, painter=False):
    ...
    depth_values = 255-((verts[m][:, 2]+.25)*400).astype(np.uint8)
    ...
    #cv2.adaptiveThreshold(out, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 5, 3,dst=out)
    cv2.GaussianBlur(out,(7,7),0,dst=out)
