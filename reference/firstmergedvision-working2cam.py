...
class AppState:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.translation = np.array([0, -1, 0], dtype=np.float32)
        self.distance = .02
        self.prev_mouse = 0, 0
...
w, h = depth_intrinsics_td.width, depth_intrinsics_td.height
...
w, h = depth_intrinsics_fw.width, depth_intrinsics_fw.height
...
while True:
    ...
    if not state.paused:
        ...
        depth_start = -cv2.convertScaleAbs(depth_image, alpha=.5,beta=-300)
        cv2.imshow('start',depth_start)
        ...
        depth_frame = frames_fw.get_depth_frame()
        color_frame = frames_fw.get_color_frame()
        ...
        depth_image = np.asanyarray(depth_frame.get_data())
        color_image = np.asanyarray(color_frame.get_data())
        depth_start = -cv2.convertScaleAbs(depth_image, alpha=.05,beta=0)
        cv2.imshow('startfw',depth_start)
        ...
        if False:
            ...
            angle_rad = np.radians(state.yrot)
