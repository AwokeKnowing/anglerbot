...
class DynamicPathfinder:
    ...
    def next_wheel_vels(self, image=None, goal=(0,0), from_vels=None, steps=2, botxy=None):
    ...
    def get_best_path(self, 
        ...
        paths = sorted([path for path in paths if path['safe']], key=lambda p: ( 
            ...
            w_direction * abs(self.angleBetweenPoints(p['points'][-1],goalxy,botxy)) / 180 +
        ...
        if False:
            ...
            for i,p in enumerate(paths):
                print("{:2}({:.2f}, {:.2f})".format(i,*p['vels']), 
                    ...
                    "direc: {:.2f}".format(w_direction * abs(self.angleBetweenPoints(p['points'][-1],goalxy,botxy)) / 180),
    ...
    def angleBetweenPoints(self,point=(1,1), target=(-1,1), origin=(0,0),rad=False):
