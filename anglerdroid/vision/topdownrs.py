...
def plane_from_points(points):
    ...
    normal = normalize(weighted_dir)

    # Convert normal to degrees_x and degrees_y
    degrees_x = np.degrees(np.arctan2(normal[1], normal[2]))
    degrees_y = np.degrees(np.arctan2(normal[0], normal[2]))

    if np.all(np.isfinite(normal)):
        return centroid.tolist(), normal.tolist(), degrees_x, degrees_y
    else:
        return None
...
class TopdownRealsenseCamera:
    ...
    def frame(self,depth_accumulate=3):
        ...
        if result is not None:
            centroid, normal, degrees_x, degrees_y = result
            #print("Centroid:", centroid)
            #print("Normal:", normal)
            print("Degrees Xy:", degrees_x, degrees_y)
            #print("Degrees Y:", degrees_y)
        else:
